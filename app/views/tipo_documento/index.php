<?php $this->setSiteTitle('Tipos de documento') ?>
...
<div class="card card-secondary">
   <div class="card-header text-center">
      <h3 class="my-0">Tipos de documento</h3>
   </div>
      ...
      <div class="table-responsive">
         <table class="table table-striped table-condensed table-bordered table-hover" data-filtering="false">
            <thead class="text-center">
               <th class="col-auto bg-info">Tipo de documento</th>
...
<script>
   ...
   function editar(id) {
      jQuery.ajax({
         url: '<?= PROOT ?>tipoDocumento/editar',
         data: {
            id: id
         },
         method: "GET",
   ...
   function guardar() {
      if ($("#frmTipoDocumento").valid()) {
         var form = jQuery('#frmTipoDocumento').serialize();
         jQuery.ajax({
            url: '<?= PROOT ?>tipoDocumento/nuevo',
            method: "POST",
            data: form,
            success: function(resp) {
               if (resp.success) {
                  ...
                  setTimeout(function() {
                     window.location.href = '<?= PROOT ?>tipoDocumento';
                  }, 2000);
               } else {
         ...
         jQuery.ajax({
            ...
            success: function(resp) {
               if (resp.success) {
                  ...
                  setTimeout(function() {
                     window.location.href = '<?= PROOT ?>tipoDocumento'; //will redirect to your blog page (an ex: blog.html)
                  }, 1500);
               } else {
                  alertMsg('Proceso fallido!', 'Ha ocurrido un error: ' + resp.errors, 'error', 2000);
                  return;
   ...
   function eliminar(id) {
      swal({
         ...
         function(isConfirm) {
            if (isConfirm) {
               jQuery.ajax({
                  url: '<?= PROOT ?>tipoDocumento/eliminar',
                  method: "POST",
                  data: {
                     id: id
                  },
